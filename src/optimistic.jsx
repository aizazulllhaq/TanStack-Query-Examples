...
const Optimistic = () => {
  ...
  const { mutate, isError, isPending, variables } = useMutation({
    ...
    // invalidateQueries : refetch posts from server
    onSuccess: async () => {
    ...
    <>
      <div className="p-4 flex gap-12">
        ...
        <div className="flex-1">
          ...
          <ul>
            {isPending && (
              <li className="border p-2 mb-4 opacity-40">{variables.title}</li>
            )}

            {isError && (
              <div className="flex justify-between">
                <li className="border p-2 mb-4 opacity-40 text-red-400">
                  {variables.title}
                </li>
                <button
